...
import type { DB, UIState, Currency, Lead, TaskItem } from "../App";
...
function OfflineTip() {
  ...
  export default function Dashboard({ db, ui }: { db: DB; ui: UIState }) {
  ...
  const revenue = revenueEUR * rate(currency);
    ...
    const totalLimit = Object.values(db.settings.limits).reduce((a: number, b: number) => a + b, 0);
      ...
      <div className="grid lg:grid-cols-2 gap-3">
        <div className="p-4 rounded-2xl border border-slate-200 bg-white">
          ...
          <div className="flex flex-wrap gap-2">
              {["Очередь", "Задержка", "Пробное", "Ожидание оплаты", "Оплаченный абонемент", "Отмена"].map((s: string) => (
                <div key={s} className="px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs">
                  <div className="text-slate-500">{s}</div>
                  <div className="text-lg font-semibold text-slate-800">{db.leads.filter((l: Lead) => l.stage === s).length}</div>
                </div>
              ))}
          </div>
        </div>
        <div className="p-4 rounded-2xl border border-slate-200 bg-white">
          ...
          <ul className="space-y-2">
            {db.tasks
              .slice()
                .sort((a: TaskItem, b: TaskItem) => +new Date(a.due) - +new Date(b.due))
                .slice(0, 6)
                .map((t: TaskItem) => (
