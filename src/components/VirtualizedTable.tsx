...
export default function VirtualizedTable<T>({ header, items, rowHeight, height, renderRow }: VirtualizedTableProps<T>) {
  ...
  }, [height, items.length, measuredHeight, rowHeight]);

  const OuterElement = React.useMemo(
    () =>
      React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>((props, forwardedRef) => {
        ...
        return (
          <div
            ...
            style={{ ...style, height: computedHeight, maxHeight: computedHeight }}
          >
            <table
              ref={node => {
  ...
  return (
    <div
      ref={containerRef}
      className="flex h-full min-h-[200px] flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900/70"
    >
      <div
        ref={topScrollRef}
